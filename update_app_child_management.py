...
child_management_code = """
// ============================================================================
// 👨‍👩‍👧 DINAMIK KO'P FARZANDLIK VA PROFIL BOSHQARUVI (LOCALSTORAGE & UNIQUE ID)
// ============================================================================
...
function handleAddNewChildSubmit() {
    ...
    alert(`🎉 Yangi farzand profil muvaffaqiyatli yaratildi! (ID: ${newId})`);
}
...
# Replace childrenDatabase definition and add handlers
js_pattern = r'// 3\. KO\'P FARZANDLIK TIZIMI MA\'LUMOTLARI[\s\S]*?let childrenDatabase = \{[\s\S]*?\};\s*let currentChildKey = "child_1";'
js = re.sub(js_pattern, child_management_code.strip(), js, count=1)

# Ensure renderChildSelectDropdown is called inside renderActiveChild or DOMContentLoaded
if "renderChildSelectDropdown()" not in js:
    js = js.replace("renderActiveChild();", "renderChildSelectDropdown();\n    renderActiveChild();")

if "checkParentOnboarding();" not in js:
    js = js.replace("initRadarMap();", "initRadarMap();\n    checkParentOnboarding();")
...
print("Updated app.js with dynamic child management, onboarding and ID system!")
