...
create table if not exists employees (
    ...
    -- Operator self-review kirish huquqi uchun (TZ 3.9-band). NULL bo'lsa,
    -- xodim botga kira olmaydi (operator rejimi o'chiq).
...
create table if not exists conversations (
    ...
    audio_file_path     text,          -- LOKAL yo'l, audio bulutga yuborilmaydi
...
comment on table conversations is 'Suhbat transkripsiyalari. Audio fayl bulutga yuborilmaydi (TZ 4.5)';
...
create table if not exists bonuses (
    ...
    conversations_count integer not null,          -- shu oydagi suhbatlar soni
    monthly_norm        integer not null,           -- hisoblashda ishlatilgan norma
    quality_coef        numeric(5, 4) not null,      -- o'rtacha_ball / 100
    volume_coef         numeric(5, 4) not null,      -- min(soni/norma, 1.0)
    bonus_max_percent   numeric(5, 4) not null,       -- hisoblashda ishlatilgan foiz (masalan 0.10)
    salary_at_calc      numeric(12, 2) not null,
    bonus_amount        numeric(12, 2) not null,       -- = salary * bonus_max_percent * quality_coef * volume_coef
...
create table if not exists self_reviews (
    ...
    self_comment        text,
    ai_score            integer,             -- solishtirish uchun nusxa (analytics.total_score bilan bir xil)
    score_diff          integer,             -- abs(self_score - ai_score)
    created_at          timestamptz not null default now()
);

comment on table self_reviews is 'Operatorning AI bahosini ko''rmasdan turib bergan o''z bahosi (TZ 3.9). Hozircha avtomatik signal yo''q — faqat statistika';
...
-- -------------------------------------------------------------------------
-- ROW LEVEL SECURITY — TZ 7.1-band: endi MAJBURIY (avval faqat tavsiya edi)
-- -------------------------------------------------------------------------
-- Bot faqat service_role kalit orqali ishlaganda, RLS "aylanib o'tiladi"
-- (service_role har doim to'liq huquqqa ega), lekin RLS'ni baribir yoqish
-- tavsiya etiladi — kelajakda anon/authenticated kalit bilan to'g'ridan-to'g'ri
-- ulanish (masalan veb-dashboard) qo'shilganda himoya avtomatik ishlaydi.
...
-- Standart holatda hech kimga (anon/authenticated) ruxsat berilmaydi —
-- faqat service_role (backend) orqali kirish mumkin. Bu — "deny by default"
-- tamoyili. Kelajakda veb-dashboard qo'shilganda, bu yerga rolga mos
-- SELECT/UPDATE policy'lar qo'shiladi (masalan: HR faqat employees'ni
-- ko'radi, lekin bonuses'ni ko'ra olmaydi — TZ 2-bo'lim).
